...
TestObject Weeks = new TestObject('Weeks')
Weeks.addProperty(
	'xpath', 
	ConditionType.EQUALS, 
	"//button[@title='week view' and text()='week']"
)

WebUI.waitForElementClickable(Weeks, 10)
WebUI.click(Weeks)

// Step 5: Verifikasi tombol week aktif (punya class 'fc-button-active')
String weekButtonClass = WebUI.getAttribute(Weeks, 'class')

if (weekButtonClass.contains('fc-button-active')) {
	WebUI.comment('✅ Tombol week aktif.')
} else {
	WebUI.comment('❌ Tombol week tidak aktif.')
